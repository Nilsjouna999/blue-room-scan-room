...
- **BR-S141 — Halo develop ritual: full audit + polish (coordinate · de-lag · de-clutter · pace · smooth)** (2026-06-29, reveal/reveal.css + reveal/card-frame.js; from a 20-agent read-only Workflow `halo-develop-polish-audit` — 7-lens audit / 8 design lanes / judge / perf+taste+freeze critics / synth; 25 edits applied + verified in the main loop). Builder feedback: the concept + effects are good but "uncoordinated, rushed, cluttery, semi-laggy, lacking sleek optimization + poise — like a katana smith." PRESERVED every effect; polished the whole arc into a deliberate "five katana beats, breath between strikes" ceremony. **PERF (the lag fix): peak concurrent filter() animations ~14 → ~3.** Stripped the per-frame `drop-shadow` from `rv-pip-ignite` (a sub-px blur on 16-20 concurrent glints — invisible-grade; kept the brightness+scale punch); widened the material-catch stagger (pip cascade 560+c*26+r*8 → 720+c*40+r*10, band-tracked) so few glints overlap; de-shadowed the corner/foil/plate catches; and moved the 40+ persistent CSS `will-change` hints to a JS LIFECYCLE (`rvCatchPromote(true)` at finalize-start, `(false)` at cleanup — backing stores live only during the ~1.3s window, mirroring the seal's add-then-clear). **COORDINATION/PACING:** spaced the finalization "strike" beats (lock delay 60→150, stamp 125→210) so impact→lock→authorize→seal→settle read as deliberate steps, not a pile; `ritualDur` 900→1360 for a real held settle after the last catch (freeze-safe — cleanup ~160ms after the edition catch). **SCANS:** the two scans (meniscus wet-edge + seal clear-coat) kept but coordinated — the seal recolored to a violet-cored clear-coat (ties to the halo light harmony; same `--rv-halo`) so they read as one material story. **EASING:** unified/named the ceremony curves. Climax preserved (tonal/bloom crest meets the final pip at 3486ms — NOT shortened). **Verified** (preview DOM + console clean): full develop→finalization→read settles to `.rv-halo-settled`, no lingering seal/classes, no cut-off; the catch `will-change` is DROPPED at rest (`auto`, not a persistent hint); `rv-pip-ignite` filters = brightness/saturate only (drop-shadow gone); all 6 catch keyframes preserved; develop END renders the clean minted card. SCOPED to the reveal (`.rv-cardframe.is-developing/.is-finalizing .rv-layer--halo`) — LIVE ROOM card untouched. ONE commit on main. **The de-lagged/coordinated/poised FEEL is the builder's LIVE call** — headless virtual-time fast-forwards the ~4.8s ritual.

- **BR-S140 — card's 4 stats back to ORIGINAL (builder's final call): only the "Stats & Readings" panel stays switched** (2026-06-29, app.js): the builder settled it — "leave the 4 stats as they were originally, just change stats and readings order." So the CARD's Frame Reading ledger is reverted to original (Frame Presence · Signal · Visual Impact · Scene Charge), undoing BR-S136; the right-panel "Stats & Readings" free read STATS stays switched (impact · signal · frame · presence, BR-S139). Verified (preview DOM). ONE commit on main.

- **BR-S139 — the free read's STATS module IS the list the builder wanted switched (re-applies the reversal; supersedes BR-S138)** (2026-06-29, reveal/reading-panel.js): the builder clarified (screenshot) that the RIGHT-PANEL free read "3 STATS" is the "first stat reading" they wanted reordered — so BR-S135's reversal was correct and BR-S138's restore was the wrong move. Re-applied the switch → **impact · signal · frame · presence** (verified). The CARD's Frame Reading ledger remains reversed from BR-S136 (also in the switched order) — pending the builder's call on whether only the right panel or both should be switched. ONE commit on main.

- **BR-S138 — restore the free read's STATS module to its original order** (2026-06-29, reveal/reading-panel.js): the right-panel "STATS" had been reversed in BR-S135 by a mis-targeted reorder (the builder meant the CARD ledger, fixed in BR-S136). Per the builder, restored the free read STATS to its original **presence · frame · signal · impact**. Verified (preview DOM). ONE commit on main.

- **BR-S137 — Halo Finalization ritual: final-pip impact → card lock → BLUE ROOM ARCHIVE blue stamp → released diagonal seal pass → per-material catches → settle** (2026-06-29, reveal/reveal.css + reveal/card-frame.js + reveal/stage-controller.js; from the builder's "Halo Finalization Motion Spec v2" via a 16-agent read-only Workflow `halo-finalization-seal` — 4 forensics / 7 competing authors / judge / law-taste + perf-freeze + integration-RM critics / synth; implemented + verified + committed in the main loop). The develop now FINISHES with a premium "authorize/coat/seal" ritual fired at the final-pip land (~3486ms). **Causal chain:** the tagged final pip (`.is-final-pip`, c4/r3 via CSS-var match) impacts (restrained 1→1.09, 1-2 frame overbright — NOT a casino pop) → the card LOCKS (micro `rv-final-lock` on `.card__plate`, scale 1.004 + a whisper of brightness/contrast, no white flash) → the **BLUE ROOM ARCHIVE** label is STAMPED (a duplicate-glyph `.card__house::after` lights the ◆ + letter edges in restrained phosphor blue via text-shadow/-webkit-text-stroke, pre-charge→hit→retract, leaving the label sharper — localized, no whole-card wash) → a blue-white NUCLEUS forms at the mark and SHEARS into a diagonal band (`.rv-seal::before`, the "released from the stamp" beat; anchor measured onto the ◆ at plate ~8%/5%) → the **HALO SEAL PASS** sweeps top-left→bottom-right (ONE injected `.rv-seal` overlay, 115° gradient, `mix-blend-mode:screen`, clipped by `.card__plate overflow:hidden` to the card surface not the −60px glow, z-index 7) → **per-material catches** fire along the sweep path (corners sharp metallic +13% · title/archetype foil +10% · photo broad gloss +5%/+3%sat · pips tiny +6% per-pip · **barcode MATTE +2.5% only, no specular** — the proof materials differ · edition plate +10% metallic close) → settle to `.rv-halo-settled` (static contrast 1.02/saturate 1.01 — subtly richer, sealed, NOT glowing). **Architecture:** the develop gate (`ddur` 3550ms) is UNCHANGED; the ritual rides a SEPARATE `.is-finalizing` class added by the controller at t=3486 (a JS timer chain) + cleaned up at the end → settled. A single `finalizeCleanup()` (idempotent: cancels timers, removes the overlay + every ritual class + the final-pip tag, drops will-change, snaps to settled) is called from setMode top, the develop restart, destroy(), AND `enterHaloReading()` (so the see-deeper→read arrow snaps the ritual). **Reduced-motion** fallback: no travel/overscale — pips seat instantly, one soft blue label tint, settle. **Repeat variant** (`.is-fin-repeat`, ~360ms): restrained pip + quick stamp + short micro-sheen, no long pass. ALL scoped to `.rv-cardframe.is-finalizing .rv-layer--halo` — the LIVE ROOM card + a plain renderCard() are byte-clean. **Verified** (preview DOM + console clean): full develop→finalization→read flow settles to `.rv-halo-settled`, no lingering seal/classes; seal = screen-blend gradient z7 clipped by plate `overflow:hidden`; `.card__house::after` stamp = "◆ BLUE ROOM ARCHIVE"; corners catch `rv-catch-metal`; nucleus measured onto the ◆ mark; LIVE ROOM card UNAFFECTED (no seal, `::after` content:none, not in `.rv-layer--halo`); reduced-motion non-moving. ONE commit on main. **The ritual FEEL (sweep cadence, stamp, catches) is the builder's LIVE call** — headless virtual-time fast-forwards the ~900ms ritual. *(Provisional/verify-live: the nucleus anchor + the seal-specular warmth over `.cardfinish`.)*

- **BR-S136 — card FRAME-READING reorder (the right list this time) + consent-modal blur fix** (2026-06-29, app.js + reveal/reveal.css): two quick polish fixes. **(1) Reorder — corrected target:** BR-S135 reversed the RIGHT-PANEL "STATS" module, but the builder meant the CARD's "FRAME READING" ledger. Reversed the renderCard ledger array `["presence","signal","visualImpact","charge"]` → `["charge","visualImpact","signal","presence"]` → the card now reads **Scene Charge · Visual Impact · Signal · Frame Presence** (verified). *(The right-panel STATS stays reversed pending the builder's call — flagged.)* This affects the LIVE card too (renderCard is shared) — intended (the ledger order is a card-design constant). **(2) Consent-modal ("No — let me see" / "Alright") blur "sharp/odd/unsatisfying":** ROOT CAUSE — the `.rv-modal` container faded its OWN `opacity 0→1` (240ms) while `.rv-modal__backdrop` carried a STATIC `blur(6px)`, so the blur snapped in with the opacity AND `backdrop-filter` inside an opacity-animating ancestor composites badly. FIX: the container no longer fades (opacity:1 + visibility-gated); the backdrop now RAMPS its own `backdrop-filter: blur(0→12px)` + scrim (0→0.5) + opacity over 480ms `--rv-ease-soft` (a soft defocus that settles in, not a slab), and the panel settles `scale(.965→1)` over 420ms soft. RM pin extended to `.rv-modal__backdrop` (snaps under reduced-motion). **Verified** (preview DOM + console clean): card ledger reversed; modal open-target = blur(12px) + scrim .5 + panel scale 1, container opacity constant 1 (no opacity-group snap). ONE commit on main. *(Modal FEEL is the builder's live call; the snap is gone by construction.)* **NEXT:** the Halo Finalization seal-pass ritual (BR-S137, in a 16-agent workflow).

- **BR-S135 — pip COLUMN-SCAN IGNITION (replaces the row-fade) + first-read stat reorder** (2026-06-29, reveal/reveal.css + reveal/card-frame.js + reveal/reading-panel.js; the ignition from a 13-agent read-only Workflow `pip-column-ignition` — 3 forensics / 6 competing keyframe+timing authors / judge / law-taste + freeze-RM critics / synth; verified + committed in the main loop). **(1) Stat reorder:** the FREE read's STATS module reversed per the builder (4→1, 3→2, 1→4) — presence·frame·signal·impact → **impact·signal·frame·presence** (reading-panel.js buildFree). **(2) Pip ignition (builder: "the current pips animation was lacking"):** BR-S134's row-major fade (`rv-pip-fill`) replaced by **`rv-pip-ignite`** — a left→right VERTICAL COLUMN-SCAN: each column lights top→bottom (42ms row stagger) with a 99ms inter-column pause (225ms column stride), starting at **2100ms** so the COLD archive diamonds (pale, low-energy, scale .92) are visible under the wipe FIRST, then ignite. Per pip: compress→**overbright flash** (brightness .62→1.90, ~1-2 frames)→a warm paper-tone drop-shadow bloom that PEAKS then RETRACTS→settle to the clean beveled minted material (scale arc .92→1.18→.98→1.04→1.00). The glow is gone at rest (no constant neon); resolves INTO the existing `.fr__pip--p1..p5` material (brass/violet/gold), not hardcoded. **Timing:** the develop gate (card-frame.js `ddur`) + `rv-halo-tonal` + `rv-fix-bloom` bumped 3000→**3550ms** so the print's fix-crest converges with the final igniting columns; last pip ends 3486ms (64ms settle before the gate) — no cut-off. `rotate(45deg)` re-stated every keyframe stop; freeze-safe END = the resting pip; RM pin adds `filter:none`. **Forensic/premium/ritual, NOT casino** (law-taste critic pass; the lone casino-risk knob = the 1.90 flash on the gold pip — judged a single ~50ms strike that retracts). **Verified** (preview DOM + headless @1600×900, console clean): `rv-pip-ignite` wired (base 2.1s, column-dominant `--c*225 + --r*42`), gate/tonal 3.55s, wipe 2.4s; develop END = minted card + seated pips + glow (no cut-off); LIVE ROOM card pips UNTOUCHED (animationName none, static — the rule is scoped to `.rv-cardframe.is-developing .rv-layer--halo`); stat reorder confirmed. SCOPED to the reveal (dev route). ONE commit on main. **The column-scan FEEL + the flash heat are the builder's LIVE call** (headless virtual-time fast-forwards the 3550ms develop — can't catch mid-scan frames).

- **BR-S134 — SIX missions in one pass: bolder halo develop · pip-fill · card-size parity · arrow hitbox · TIER 6 robustness · TIER 7 a11y** (2026-06-26, reveal/{card-frame,reveal.css,reading-panel,stage-controller,arrow-button,warning-modal}.js/css + app.js; from a 45-agent read-only Workflow `reveal-multimission` — 6 forensics / 27-author design / 5 judges / 3 adversarial critics / 3 synth-patches; I implemented + verified + committed in the main loop, read-only-agent law). **M1 — halo reveal genuinely bolder (builder: "dull, bit fast, didn't improve"):** the develop went 1850→**3000ms** (card-frame.js); rv-halo-tonal rebuilt as a 5-beat **latent-surfacing** arc (deep ghostly br1.32/co0.52/sa0.28 → true), the violet fix-bloom withheld until **82–90%** so the glow builds as the print "fixes" (spread 34→44px), the wet-meniscus body+crest strengthened, wipe+meniscus slowed to 2400ms — a savored ceremony, still GPU-only + glow-clip (`inset(-60px)`) preserved + freeze-safe (verified). **M2 — Frame-Reading pips FILL row-by-row, L→R:** new `rv-pip-fill` keyframe + per-pip inline `--r/--c` (in renderCard), `animation-delay = --r*150ms + --c*55ms`, SCOPED to `.rv-cardframe.is-developing .rv-layer--halo .fr__pip.is-on` so it never fires on the live-room card or a plain re-render; completes ~1030ms inside the develop. **M3 — card-size parity:** the fullview card was `zoom:1` (430px) vs the menu `zoom:0.92` (396px) — it GREW on see-deeper; pinned fullview to `width:430px; zoom:0.92` → **396px in both** (verified). **M4 — arrow hitbox "odd after first click":** ROOT CAUSE (measured) = after the slide the read panel z-stacked OVER the arrow (the arrowslot's z-index:7 trapped in `.rv-stagemain`'s transform stacking context; `.rv-readslot` a later sibling) — half the glyph was unclickable. Fix: lift `.rv-stagemain` above `.rv-readslot` (z 2/1) + crop the SVG viewBox to the ink + tight 2px box/pinned transform-origin. The workflow's M4 diagnosis missed the stacking; my independent hit-test caught it. **M5 — TIER 6 robustness:** threaded `src` into ReadingPanel (`{reading, src}`), killed every SRC-01 hardcode — comet polygon GENERATED from `src.frame.signature.radii` (9 radii @ 40°, verified SRC-01 reproduction), photo `src.file`, serial `src.card.serial`, oracle `src.dossier.oracle.short`, stat pips `statPip(src.card.stats[k]/20)`, portrait `--photo-ratio:3/4`; **also fixed a live bug** (free-read stats referenced non-existent keys clarity/intention → NaN pips). Proven per-source on SRC-02 (Ice Field: own comet/serial/oracle/portrait). **M6 — TIER 7 a11y:** aria-live on `.rv-readslot`, focus flow through develop/morph/read (guarded "only if focus already in the reveal" — never steals on page-load), Esc returns from fullview, modal `aria-labelledby`→kicker, disabled-arrow no ring, back button `visibility:hidden` when not fullview. **Adversarial critics caught the M5↔M6 `mountPanel` collision** (both rewrote it → merged into one). **Verified** (headless @1600×900 + preview DOM, console clean): per-source read (SRC-01 + SRC-02), card 396px both states, arrow glyph fully clickable (glyph-centre hits the arrow), pip-fill animation applies, develop glow-clip preserved, reduced-motion snaps to the finished card+read incl. comet, live room card unaffected, NO focus-steal at init. LIVE menu/room UNTOUCHED (dev route). ONE commit on main; not pushed without approval. **The menu add-on audit Tiers 1–7 are now ALL shipped.** *(M1 develop FEEL + M2 row-by-row are the builder's live call — headless virtual-time fast-forwards the develop.)*

- **BR-S133 — TIER 5: scribble-read MINIMAL polish (the builder LOVES it — value-level only)** (2026-06-26, reveal/reveal.css + reveal/reading-panel.js; from a 44-agent read-only Workflow `tier5-scribble-polish` — 6 forensics / 27-proposer tournament across 5 knobs / 5 judges / 5 adversarial guards / 1 synth; I implemented + verified + committed in the main loop, read-only-agent law): 5 surgical knobs, NO redesign (the minimalism guard re-grepped + confirmed zero overreach). **K1 draw-easing:** the sketch stroke draw-on `.is-writing .rv-sketch .d` gets a NEW `--rv-ease-draw: cubic-bezier(0.25,0.46,0.45,0.94)` (organic hand-drawn pen-lift), isolated from `--rv-ease` so unrelated fades don't re-ease. **K2 count-scaled stagger:** new `--rv-stagger` token = 460ms FREE (≤4 mods) / 360ms HALO (`.rv-read[data-engine="halo"]`, 8 mods) — the three `* 320ms` module-delay literals → `var(--rv-stagger, 460ms)`; reading-panel.js completion timeout mirrors it (`engine==="halo"?360:460`, 2400ms buffer kept) so modules settle before the next begins + the longer halo read doesn't drag. **K3 rv-settle:** new 3px `rv-settle` keyframe (half rv-fade's 6px) on `.rv-peak`/`.rv-stat`/`.rv-photoframe img` so pips/peak/photo ease in without a mechanical jump; text stays on rv-fade. **K4 flourish DRAWS:** the headline flourish's static `stroke-dashoffset:0` override → a freeze-safe TWO-rule pattern (default-drawn so a frozen timeline is never blank + `.is-writing …` draws it IN-sequence at the module delay) — fixes the long-static flourish without re-stranding it (the BR-S125 hazard). **K5 cap:** `.rv-cap__em` 18→17px. The contested L974–983 region was merged into ONE edit so the knobs don't revert each other. **Verified** (headless @1600×900 + preview DOM, console clean): free read `--rv-stagger` 460 + `.d` easing cubic-bezier(.25,.46,.45,.94) + flourish anim rv-draw → dashoffset 0 (draws, not stranded); halo read `--rv-stagger` 360 (8 mods) + cap 17px; free(4) + halo(8) draw fully; reduced-motion snaps BOTH to the finished read incl. the flourish drawn (no blank); the BR-S132 arrow placement intact. LIVE menu/room UNTOUCHED (dev route). ONE commit on main; not pushed without approval. **NEXT (Tiers 6–7):** T6 robustness (thread `src` into the panel — kill the SRC-01 hardcodes; comet polygon from `src.frame.signature.radii`) · T7 a11y + edge cases.

- **BR-S132 — develop-moment CRAFT (TIER 4 of the menu add-on audit) + the arrow placement nit resolved** (2026-06-26, reveal/reveal.css + reveal/card-frame.js; from a 17-agent read-only craft Workflow `tier4-develop-craft` — 3 haiku forensics / 10 competing authors / 2 judges + adversarial critic + synthesizer; I implemented + verified + committed in the main loop, read-only-agent law): two transitions upgraded from flat fades to darkroom "develop" motion, plus the arrow moved per the builder's pick. **(1) ARROW (builder chose "right of card, points into the read", caption dropped):** the menustage arrow left `position:absolute; bottom:-56px` (centred below the card + a caption underneath — the builder's "pains my soul") for `left:100%; top:50%` — it rides `.rv-stagemain`, nestling at the card's RIGHT edge, vertically centred, pointing into the read; caption hidden on the menu; glyph 60→56px. The menu seam is only ~24px so `.reveal-stage--menustage.is-reading .rv-read` gets `padding-left:52px` (a left lane) — verified the arrow (svg right 1101) clears the read content (left 1108), no module overlap (it had been landing on the "2 DIAGRAM" module). **(2) image→card (photo→free; standalone `?dev=staged-reveal` ONLY — the menu starts on FREE so it never shows this):** the flat 820ms crossfade → a 3-beat "Print Rising from the Bath" (~1180ms) — a bottom-anchored clip bloom (`inset(0 0 100% 0)→inset(0)`, lands ~62% on --rv-ease) + an exposure FIX (filter washed-out→true, back-loaded on the new --rv-ease-soft) while the BATH behind dims/desaturates/softens (blur on the SINKING photo only, never the hero)/drains (the existing ::after wash). New `is-developing-photo` branch in card-frame.js (reuses the module timer — no race). **(3) card→halo (BOTH routes — standalone see-deeper + the menu fullview):** the develop wipe gained a TONAL RISE (the SOLE filter writer on the halo layer — pale/flat→true with a +saturate swell at 84%, like fixer setting), a HALO-FIX BLOOM (a brief violet glow swell at 82% on a dedicated `::after` so it never touches the layer's filter or the −60px glow clip), and a WET-MENISCUS scanline (rv-scan→rv-meniscus: a glossy wet leading edge — fluid body + a near-white crest lip — riding the seam; the freeze+glow-safe alternative to a layer mask). Robustness: the wipe `from` clip `inset(…900px…)`→`inset(-60px -60px calc(100% + 60px) -60px)` (survives any card height). Two-easing motion language (--rv-ease + new --rv-ease-soft). **The adversarial critic earned its keep:** it caught the photo→free winner shipping a STUB (no real code) and the proposed feather-MASK FREEZING BLANK + killing the −60px glow clip — both replaced with freeze-safe forms (the no-animation END state is always the finished card; visibility is never gated on an entrance animation). **Verified** (headless Chrome @1600×900 + preview DOM, console clean): menu fullview develop END = minted halo card + glow (`clipPath inset(-60px)` preserved) + 8-mod read + ← Back button; standalone develop END clean; photo→free ends on a clean free card; reduced-motion snaps BOTH transitions to the finished card/read instantly, glow intact; the arrow nestles in the read lane with no overlap, flips to the purple "See deeper" variant, caption gone; `--rv-ease-soft` resolves. The LIVE menu/room UNTOUCHED (dev route). ONE commit on main; not pushed without approval. **NEXT (Tiers 5–7 of the spec, workflow output `…/tasks/wsc60yo20.output`):** TIER 5 scribble MINIMAL polish · TIER 6 robustness (thread `src` into the panel — kill the SRC-01 hardcodes; comet polygon from `src.frame.signature.radii`) · TIER 7 a11y + edge cases. *(The mid-develop FEEL is the builder's live call — headless virtual-time fast-forwards short animations, so it can't catch mid-frames.)*

- **BR-S131 — Menu add-on REBUILT on the real menu grid (23-agent audit plan, Tiers 1–3): starting frame = the original exactly · free read fits right (doors stay) · see-deeper → fullview** (2026-06-26, app.js + reveal/stage-controller.js + reveal.css; from a 23-agent read-only audit/design Workflow — `menu-addon-polish-audit`): the hand-rebuilt `.menurev2` chrome (cause of the 2-base-colour + card-too-left + doors-not-centred bugs) is REPLACED by hosting the REAL `.menu__inner` grid on a non-view-gated `.menurev` wrapper (the `.menu` class is `display:none` in dev view) → the starting frame is identical to the original BY CONSTRUCTION and the doors are structurally immovable (the fixed 360px grid track). New **`menustage`** reveal mode: NO own background (the body's gradient is the ONE base — 2-colour bug fixed), the sample card (`430px` × `zoom 0.92`, == `.msample__card`) lives in the menu's 1fr stage cell and starts as the developed **FREE card** (matches the original sample). **TIER 2:** press → the card slides LEFT (`translateX` derived from the read width + gap, not a magic px) + the free read (4 modules) slides in on the RIGHT; the left doors UNMOVED (verified: card at track-left 637–1032, read fits 1056–1380 in the 748px right track, no overflow). **TIER 3:** "see deeper" → modal → confirm → `opts.onFullview` promotes `.menu__stage--reveal` to a fixed full-bleed layer, the menu chrome (head+controls) steps aside (opacity 0 + translateX −40), the card develops to halo + the 8-module read fills the right; a **← Back to the menu** button + `toFree()` returns to the in-menu free read. **Verified** (DOM + headless @1600×900, console clean): start frame ≈ original side-by-side (one base · doors centred · free card right · full content/seal/foot restored); full flow free-card → free-read(in-menu) → see-deeper → fullview(chrome opacity 0 + back visible) → halo(shiny) → 8-mod read → BACK (chrome opacity 1, free read restored); zero errors; `haloTimer` guards rapid taps. Live menu UNTOUCHED (dev route). Pushed. **NEXT (Tiers 4–7 of the spec):** image→card + card→halo cinematic craft, scribble MINIMAL polish, robustness (thread `src` into the panel; kill SRC-01 hardcodes — comet polygon from `src.frame.signature.radii`, pips from `src.card.stats`, photo `src.file`, etc.), a11y + edge cases. (The full build spec is in the workflow output: `…/tasks/wsc60yo20.output`.)

- **BR-S130 — Integrated menu: the slide brought INTO the menu — brand+doors step aside, card glides left, read fills right** (2026-06-26, app.js + reveal/stage-controller.js + reveal.css; builder confirmed the BR-S129 slide ("yes"), asked to bring it into the menu): rebuilt `?dev=menu-reveal`. The menu's brand + thesis + two doors are now a LEFT chrome layer (`.menurev2__chrome`); the staged reveal mounts BARE (new `opts.bare` — skips the reveal's own header, KEEPS the horizontal slide, distinct from `opts.embedded`'s vertical stack) into `.menurev2__stage` (full-width). On develop (the reveal's new `opts.onReading` fires at the read step), `.menurev2` gets `.is-developing` → the chrome FADES + steps aside (opacity 0 + translateX −22px) so the card has room to glide LEFT (the BR-S129 slide) and the read scribbles in on the RIGHT. **Verified** (DOM + headless): init = chrome opacity 1 + card centred (tx 290) + NO duplicate header (bare); develop = chrome opacity 0 + card tx 0 (left) + read on the right + no page overflow; console clean. The LIVE menu is still UNTOUCHED (dev route). Pushed. **OPEN/NEXT:** photo-state polish (the card sits a touch tight against the doors before develop); decide brand-stays-as-small-header vs full fade; a way back to the menu after develop; then promote to the real menu.

- **BR-S129 — Staged Reveal: cinematic card-slide-LEFT + the read scribbles in on the RIGHT** (2026-06-26, reveal/stage-controller.js + reveal.css; builder: "slide it smoothly and cinematically more toward the left the card, and scribble comes to the right side of it"): the card sat still while the read appeared beside it. Now the card+arrow (wrapped in `.rv-stagemain`) sit CENTRED in photo/free (`transform: translateX(min(290px,21vw))`) and GLIDE LEFT to `translateX(0)` the moment the read appears (`.reveal-stage.is-reading`, set in `enterFreeReading`), 820ms house ease; the `.rv-readslot` reserves its width (`min(560px,44vw)`) so the slide is pure GPU transform (no reflow), and the read fades + slides in from the right (opacity + translateX, +220ms delay). The card stays left through the halo (is-reading persists; the read clears + redraws on the right). Embedded (menu) mode keeps its vertical stack (slide overridden). Reduced-motion: no slide, read shown. **Verified** (DOM + headless): photo/free tx=290 (centred) → reading tx=0 (left), caught mid-slide at 11; free read renders on the right; halo holds card-left + 8 mods; console clean. Pushed. **NEXT:** bring the slide into the MENU integration (the menu chrome recedes so the card has room to slide left + the read fills the right) — `?dev=menu-reveal` still uses the vertical embed for now.

- **BR-S128 — Integrated-menu PREVIEW (`?dev=menu-reveal`): the menu's sample card "develops in place" (staged reveal embedded)** (2026-06-26, reveal/stage-controller.js + reveal.css + app.js; builder asked to see the menu WITH the new system as a whole, chose "sample card develops in place"): added an **embedded mode** `BRReveal.mount(root,{embedded:true})` (no own header — the menu's brand serves; vertical stack via `.reveal-stage--embedded`). Preview route `?dev=menu-reveal` renders the REAL `renderMenu()` markup (in a non-view-gated `.menurev-wrap` — `.menu` is `display:none` unless `view=menu`) and swaps the `.msample__card` slot for the embedded reveal, so the sample card develops in place; the menu's brand/thesis/two doors stay intact on the left (left column top-pinned by dropping the menu grid's top `1fr` spacer row). **The LIVE menu is UNTOUCHED.** OPEN: the developed-state layout (superseded by BR-S129's slide direction — the menu version will be re-done so the chrome recedes + the card slides left). Pushed (dev route only).

- **BR-S127 — Staged Reveal: the free→paid moment reframed as a slow "develop" (the minted card WIPES IN over the free card) + beat-sequenced** (2026-06-26, `reveal/card-frame.js` + reveal.css + stage-controller.js; builder: "image→card is fine, but free→paid is too sudden — make it slower, enjoyable, the card being built/revealed"): the free→halo card change was an 820ms opacity crossfade (felt abrupt). Now it's a **develop wipe** — the minted (shiny) card layer reveals **top→bottom over the free card beneath** via an animated `clip-path: inset()` (~1500ms), with a brighter purple **developer bar** (`.rv-scanline`) riding the reveal edge; since both layers are the SAME card geometry, only the treatment (matte→minted + halo glow) develops, like a print coming up in the tray. The glow is preserved (`.rv-layer--halo` base `clip-path: inset(-60px)`; the wipe animates within it). **Beat-sequenced** in `enterHalo`: the free Stats & Readings fades to empty FIRST, then a 320ms beat, THEN the card develops (~1500ms wipe + ~350ms settle before the arrow re-enables) — so the free→paid moment is ~2.2s of paced ceremony, not one sudden swap. Compositor-cheap (clip-path + transform + opacity only); reduced-motion snaps the halo in (no wipe). **"image→card" (photo→free) untouched** per the builder. **Verified** (live DOM drive, console clean): mid-develop = BOTH layers shown + `clip-path` animating (e.g. `inset(-60px -60px 218px)`); settles to halo-only/shiny cleanly; then 8-mod halo draws; no errors. ONE commit on main; pushed. **NEXT:** builder to judge the develop feel live (duration/scan-bar tunable); final copy; promote to menu.

- **BR-S126 — Staged Reveal polish: arrow dim→hover, readings CLEAR on the halo transition, slower/moodier pacing, + Frame-Signature (comet metric) & Source-photo modules, bug+pacing+a11y audit** (2026-06-26, `reveal/*` — reading-panel.js + reveal.css + stage-controller.js + arrow-button.js; builder polish round): **(1) Arrow** dim at rest (`--rv-ink-dim`) → brightens to `--rv-ink` on hover. **(2) Halo transition** — pressing "No — let me see" now CLEARS the free Stats & Readings to empty (a 280ms fade then wipe, `ReadingPanel.clear()` called from `enterHalo`) AS the card morphs to halo; the next press draws the halo version down. **(3) Pacing** slowed + given mood/spacing: module stagger 150→**320ms**, sketch draw 0.55→**0.85s**, per-stroke 42→**60ms**, caption offset 470→**1050ms** (lands near sketch completion, not 0.6–0.9s early), so ~3 modules draw at once instead of 5–6 — deliberate, savored, still ~3.5–4.5s. **(4) +2 HALO modules:** **FRAME SIGNATURE** = the real "Comet — a single spike" metric (silhouette drawn from the actual `frame.signature.radii`) + **SOURCE** = the real `assets/source-01.jpg` in a hand-drawn corner-marked frame. Halo now 8 modules (2-col × 4 rows). **(5) Audit (2 read-only agents — bug + pacing/lag/a11y):** fixed a HIGH race (the `clear()` wipe-timer was uncancellable → a fast tap could blank the halo read; now stored in `clearTimer`, cancelled in `setReading`/`play`/`clear`); converted the comet `<polygon>` + signal `<polyline>` → `<path>` (Safari ≤15.3 `pathLength` draw-on); raised the arrow focus-ring contrast (50→78%); arrow gets `tabindex=-1` while `aria-disabled`; source `<img>` got a real alt. Lag audit = clean (transform/opacity/stroke-dashoffset/filter only); reduced-motion covers all new elements. **Verified** (live DOM drive + headless captures, console clean): full flow; free 4 mods → CLEAR to empty at HALO → 8-mod halo draws; clear-race holds (panel opacity 1 after a rapid double-press, not blanked); comet is a path; arrow dim/hover/gone correct. NOTE for future captures: the `?rv=` auto-drive needs a ~23–25k `--virtual-time-budget` now (slower pacing); over-long budgets stall it. ONE commit on main; pushed (builder wants it live). **NEXT:** final copy; per-sketch tuning; promote to menu.

- **BR-S125 — Staged Reveal readings redesigned to the builder's hand-drawn "archive sketchbook" + arrow recreated as a scribble SVG, moved right-of-card** (2026-06-26, `reveal/*` — reading-panel.js REWRITE + reveal.css + arrow-button.js + stage-controller.js; on the builder's mockup + 2 arrow prototypes): the BR-S124 document-style readings were replaced with the builder's vision — a hand-drawn FIELD-NOTEBOOK. **FREE = 4 modules** (Surface · Diagram · Stats · Signal Note); **HALO = 6 modules in a 2-col grid** (Surface Record · Archetype · Aura · Mint Record · Oracle Read · Map/Position) — picture-close to the mockup; content is decorative SHOWCASE (builder: "stats need not be accurate"). Each module = a bespoke inline-SVG sketch (compass, aura rings, wax seal, crosshair quadrant, framed hand-to-lens diagram, waveform, swatches, sparkles) + handwritten Cormorant-italic labels. **The sketches DRAW THEMSELVES IN** (stroke-dashoffset on every `.d` mark, per-path `--di` stagger) + labels fade, sequenced module-by-module via `--i` — "scribbled down as you press the arrow." **Arrow** recreated as a crisp hand-drawn scribble SVG (curved + hatched + arrowhead — winner picked from 3 variants rendered large in a throwaway harness), a BARE glyph (button chrome dropped), moved from below the card to the **RIGHT of the card, a touch below centre**; **silver normally → purple-neon for the FIRST "see deeper" offer** only (the press that opens the modal), per the builder's answer. **9 sketches authored via 3 READ-ONLY agents** (1 sonnet circles/waves + 2 opus radial/emblem, shared draw-on spec); I compacted them (shared stroke styling → CSS), integrated, sized, brightened, and refined in the main loop. **Builder ruling: the current main menu is PERFECT — this is an ADD, not a new menu** (stays isolated on `?dev=staged-reveal`). **Verified** (live DOM drive @1440 + headless `--virtual-time-budget` captures, console clean): full flow PHOTO→FREE→FREE_READING(purple)→modal→HALO→HALO_READING; arrow silver → purple-only-at-the-offer → gone at journey end; free 4 mods / halo 6 mods swap; every module sketch draws (dashoffset 0); the headline flourish fixed (was stranded outside the write-in); mobile reflows to a single column (halo 2-col→1-col), no real h-overflow (the 4px = vertical-scrollbar gutter). Copy PLACEHOLDER. ONE commit on main; not pushed. **NEXT (builder):** final copy/labels; tune any individual sketch; promote to the menu when ready. Cleanup debt: the BR-S124 `.rv-row`/`.rv-meter`/`.rv-slot`/`.rv-dossier` CSS + the `readings.data.js` reserved/developed `dossierSlots`/`coreMeters` shapes are now UNUSED by the sketchbook panel (a future sweep removes them; the panel only reads `reading.engine`).

- **BR-S124 — Staged Card Reveal (photo→free→reading→modal→halo→reading) · 7 isolated units · isolated dev surface** (2026-06-25, new `reveal/*` + index.html + app.js; pre-edit = a read-only research/token-lock pass + the mandatory critique): built the builder's multi-agent build spec, ADAPTED to the vanilla stack (no React/bundler here). **Seven swappable units under `window.BRReveal`** (props-in/events-out; a child never knows the global stage): `ArrowButton` (grey/purple variant, bent arrow, pulse PAUSES on hover), `CardFrame` (reuses the REAL `renderCard` free/shiny as opacity-crossfade layers stacked in ONE grid cell → identical **360px** box every state; free→halo scan sweep), `ReadingPanel` (the write-in ripple + reserved-scribble vs developed-ink dossier; fed `freeReading` THEN `haloReading` — SAME component, two datasets, SWAP-not-augment), `WarningModal` (body-level, backdrop blur, Esc/outside-click/focus-trap), `StageController` (the state machine I own + wire). **Hosted ISOLATED on `?dev=staged-reveal`** (the proto-cards pattern) — NOT on the live menu yet; protects the BR-S123 card; promotion is a one-line follow-up. Tokens MAPPED onto the existing system (no fresh palette): `--ink-950`/`--t-display`/`--violet`; mono = IBM Plex Mono 400/500 (never 600+, the faux-bold gotcha). **Law hold (free-as-hero):** purple appears ONLY after the free read settles, the pulse is slow + pauses on hover, "Alright" is the easy default, the modal says the Free Card is already complete; MOCK paywall only (no real payment/engine, like the existing dossier). A read-only law audit = CLEAN (artifact-only copy, no person-read, no 0–100). **Agents:** 1 Sonnet research/token-lock + a parallel READ-ONLY review fan (Haiku lag/CSS/a11y = all PASS · Sonnet bug/stress + law = clean); I authored + wired + verified + committed in the main loop (read-only-agent law). **Hardening applied** from the bug fan: modal repeat-open guard, orphan-keydown self-heal on re-mount, stale write-in timer cleanup, autoAdvance escape, and the real one — the write-in no longer depends on `requestAnimationFrame` (throttled in bg tabs / paused in preview → would strand rows invisible); now a forced reflow + synchronous class add. **Verified** (live DOM drive @1600×900 + headless capture w/ `--virtual-time-budget`, console clean): full PHOTO→…→HALO_READING flow; frame fixed **360×707** every beat; purple only post-settle; modal leaves the card unchanged; free→halo swaps readings (4 reserved → 5 developed slots); arrow fades at journey end; triple-click advances EXACTLY one stage; modal survives open/Esc/backdrop/double-confirm spam; mobile reflows to a column with ZERO page overflow (card zoom 0.82 ≤430px + stage `overflow-x:clip`); reduced-motion block covers every animated selector; the live room/menu/card are UNTOUCHED (zero `.reveal-stage` leak; room still renders 1 card + 5 dossier plates). Copy is PLACEHOLDER (builder writes final words). ONE commit on main; not pushed without approval. **NEXT (builder calls):** (a) drop final copy into `reveal/readings.data.js` + the modal; (b) promote to the live menu (one-line) when ready; (c) wire the real paywall behind the modal's "No — let me see".

- **BR-S123 — Mint Record showcase from the card + 5 polish/flow fixes (spec'd via a 3-lens read-only panel)** (2026-06-25, app.js + index.html + styles.css): **(C) Mint Record showcase:** a mono "Mint Record" link sits beside the barcode (in a new `.mintstrip__right` cell; minted-only, ghosted+inert on free, JS also guards `data-treatment==="free"`). Click → the REAL dossier `.dplate--mint` goes `position:fixed` mid-screen (`top/left:50%; translate(-50%,-50%); z 901`) over a body-level `.mintscrim` (`#mintScrim`, z 900, `backdrop-filter: blur(7px)`, fades 260ms) — the page blurs, the plate stays sharp. Dismiss (outside-click / Esc) un-blurs + smooth-scrolls to the TOP. ONLY the live card triggers it (`#stageZone .card`); the real dossier plate is inert. QR ↔ Mint are mutually exclusive; `render()`/`applyTreatment()` force-close (the dossier rebuilds). **Entrance is scale-only (no opacity-0 keyframe)** so a paused timeline can never freeze the plate invisible. **(F) QR dismiss:** was a sudden `display:none` snap; added an `.is-qr-closing` reverse fade/scale (`cardqr-out`/`cardqr-seat-out` ~190ms) kept mounted via a 230ms timeout, re-open-safe, reduced-motion instant. **(A) Barcode glow:** the barcode (QR trigger) now emits light on hover/focus — `filter: drop-shadow` bloom off the silver bars; killed on the free ghost + reduced-motion. **(B) Surface Record %:** the per-swatch percent moved from chunky IBM Plex Mono 11px to elegant **Cormorant Garamond 500 / 15px** (the builder's "different finer font / kanji-style" — no thin weight is loaded, so finer = a delicate serif numeral). **(D) Signal Note audit:** kept the (strong) copy + the BR-S122 silver lift, but DROPPED the box (border+bg) — it competed with the Frame Reading ledger for "the important block"; the note now reads as the card's single VOICE via bright silver italic, no container. **(E) Signature moved** from below the ledger to DIRECTLY under the Signal Note (inside the titleblock), so the two italic-serif voice lines form one stanza — flow is now label(title/archetype) → voice(note+signature) → data(ledger) → provenance(mint strip). **Verified** (headless 1600×900 + DOM + a temp auto-open capture, console clean): showcase plate opacity 1, EXACTLY centred (450/450), "Mint Record" content sharp over the blurred page, dismiss→top; QR dismiss closing-state ~230ms; QR↔Mint exclusion; free blocks both triggers (ghost + pointer-events:none + tabindex −1 + JS guard); Surface % Cormorant 15px; Signal Note border 0; signature stacked in titleblock. ONE commit on main.

- **BR-S122 — three card polish fixes: de-bulk the tier words (faux-bold root cause), barcode-as-QR-trigger, brighter Signal Note for flow** (2026-06-25, app.js + styles.css): **(1) Tier words "bulky/unsightly":** ROOT CAUSE = IBM Plex Mono is only LOADED at weights 400/500 (index.html), so BR-S121's `.fr__tier` 600 + `--peak` 700 were browser FAUX-BOLDED (thick/ugly, worst on the long "CHARGED"). Held to real loaded weights — `.fr__tier` 10px/600/0.2em → **9.5px/500/0.13em**, `--muted` 500→400, `--peak` dropped the 700 (the gold colour crowns Peak, not a synthetic bold). Crisp now, the tier still echoes the pip ramp by colour. **(2) Barcode is the scan-code trigger (not a separate QR icon):** the builder wanted the existing bottom-right BARCODE to be the clickable thing that pops the QR (the QR pop-out design stays — they liked it). Removed the `.cardqr-trig` SVG-QR button + the `.mintstrip__codes` wrapper; the `.barcode` is now a real `<button data-card-qr>` (minted-only, ghosted + `pointer-events:none` on free, `tabindex` synced in patchCardFront). The `.cardqr` pop-out seat is unchanged. **(3) Signal Note brighter (flow):** it sat dim (`--muted` #8c867b) — a dead valley between the bright title and the ledger that "killed the flow on the middle". Lifted `.verdict` colour **--muted → --silver (#c8c4bb)**, the `.verdict__key` --faint → --silver-dim, and the box border --line-faint → --line, so the note carries presence and the card reads as one coherent flow top→bottom. **Verified** (headless 1600×900 + DOM, console clean): tier words 9.5px/weight 500 (never 600/700); the barcode is a `<button>` with data-card-qr and clicking it opens the QR seat (outside-click/Esc still close); free barcode ghosted + `pointer-events:none` + tabindex −1; Signal Note renders silver; QR pop-out unchanged. ONE commit on main.

- **BR-S121 — Frame Reading audit + refine: 3-step brass value-ramp + faceted-gem pips, tier-word ramp, and a re-centred whole-feeling ledger** (2026-06-25, app.js + styles.css; pre-edit = 3-lens READ-ONLY colour-theory/typography/layout AUDIT). Refines BR-S120's ledger on the builder's three asks. **Pips (colour theory):** the flat single base became a 3-step brass VALUE ramp climbing toward the gold finish — `--fr-1 #a07c4c → --fr-2 #bd9259 → --fr-3 #d6aa66` (each at 93% brass + a whisper of `--halo-a`, decoupled from the old 84% material-mix that dulled brass to clay on a teal card), driven per-position (`.card[data-treatment="shiny"] .fr__pip--p1/p2/p3.is-on`). Every lit pip gained a faceted 135° gradient (light top-left facet · base mid · shadow bottom-right) over the bevel insets, so the rotate-45 diamonds read as cut gems not flat tiles. The 4th purple lifted to #8a72c4 (richer chroma, the cool dip), the 5th gold deepened to #fbcb52 (the warm crown), with a climbing glow hierarchy (base none · purple 5px · gold 7px). Off-pip rim bumped 0.12→0.17 so the empty setting holds its shape. **Tier words (CHARGED/STRONG/PEAK):** kept IBM Plex Mono (instrument voice; serif is reserved for the title/verdict/signature), tightened to 10px/600/0.2em/line-height1/tnum, and given a restrained 5-step `--fr-tier` ramp that ECHOES the pips — Muted #82807a/500 · Clean silver-dim · Strong #c2a274 · Charged #b6a6e0 · Peak #ecc06f/700 (weight carries the climb, saturated colour only at the top tiers) — so word + pips state the tier in two agreeing registers. renderCard now emits all 5 tier classes (was charged/peak only). **Layout (the "offbeat / right-clustering" fix):** root cause was `.fr__row { grid-template-columns: 1fr auto 80px }` — the elastic 1fr shoved the name hard-left, opened a chasm, and slammed pips+tier to the right rail. Replaced with a fixed `116px 72px 1fr` grid inside a `max-width:300px; margin-inline:auto` band: name left, pips DEAD-CENTRE on the card axis (`justify-self:center`), tier closing a narrow band, header centred — the four rows now read as ONE whole unit on the same centre line as the title/verdict, not a sprawl. **FREE stays matte:** the free gate flattens every lit pip to plain silver (no facet, no warm) and neutralises the tier ramp to silver-dim (pip COUNT + WORD still carry the tier). **Verified** (headless 1600×900 + DOM): shiny base ramp steps #a07c4c→#bd9259→#d6aa66, all pips faceted, Charged #b6a6e0/600 + Peak #ecc06f/700; ledger a 300px centred block (centring delta 0); free pips flat #c8c4bb + tier silver-dim; console clean. QR pop-out (BR-S120) untouched. Develop reveal + reduced-motion + lab-material `--fr-on` retints key off `.fr__pip.is-on`, still valid. ONE commit on main (with BR-S119/S120, pending the builder's push).

- **BR-S120 — Halo pips reworked to the mockup (position-based: brass base + 4th purple / 5th gold + glow) + clickable QR pop-out "seat" on the card** (2026-06-25, app.js + styles.css; pre-edit critique = 2-lens READ-ONLY panel, both ship-ready): TWO builder asks. **(1) Halo pips → the builder's mockup (ScanCardFront SLOT):** switched the Frame Reading pips from TIER-based (only the last lit pip coloured by the row's tier) to POSITION-based — positions 1-3 = ONE warm brass base across all readings (`.card[data-treatment="shiny"] .framereading{--fr-on: color-mix(#c99c5e 84%, var(--halo-a))}` — brass with a faint material breath; was the silver-material tint), the 4th pip purple (#8378b0) + glow, the 5th gold (#ffd566) + glow, each only when LIT (`.fr__pip--p4.is-on`/`.fr__pip--p5.is-on`). A Peak row now shows base·base·base·purple·gold; a Charged row base·base·base·purple — the "1 colour across the readings unless anomaly" the builder asked for. FREE stays matte/bland (the free gate neutralises both accents to silver). Tier WORD colours kept (they reinforce the anomaly). The develop reveal + reduced-motion snap + lab-material `--fr-on` retints all key off `.fr__pip.is-on` and stay valid. **(2) Clickable QR pop-out:** a small placeholder-QR `<button data-card-qr>` joins the barcode in the mint strip (grouped in `.mintstrip__codes`; active on minted, ghosted+inert on free, hidden on the menu/proto reuses). Click it → a `.cardqr` layer (z6, above `.cardfinish`) blurs the card front to its OWN base colour (a near-opaque card-base gradient over a `backdrop-filter` blur) and seats a sharp placeholder QR (inline SVG) dead-centre with a caption — "giving the QR the seat", sleek, inside the card's rounded clip. Click beside the QR / anywhere else / Esc → closes; a second tap toggles. Wiring mirrors the lightbox: open is hard-scoped to `#stageZone .card` (so the reuses stay inert) + early-returns during `.is-developing`; the outside-click/Esc document listeners attach on `setTimeout(0)` (so the opening click can't self-close) and tear down on close (no leak); `render()` + `applyTreatment()` force-close so no stale QR survives a rebuild/re-skin; reduced-motion kills the pop animation. Placeholder QR for now (real scan link later). **Verified** (headless 1600×900 + DOM across menu / src1·src5 / free·shiny·mint; console clean): shiny pips brass+purple(4th)+gold(5th, glowing on Peak); free pips matte silver; QR opens→seat+blur, seat-click keeps open, outside-click/Esc/treatment-flip close, menu trigger hidden; develop CTA flips + pips reveal, no error. ONE commit on main. **Flagged (one-line tweaks if wanted):** QR is minted-only (ghosted on free — could enable on free); brass base carries a faint material breath (could go flat brass).

- **BR-S119 — New card front ported (builder's ScanCardFront design): "Frame Reading" pip-ledger + sealed finish, original halo kept, fit held to existing footprint** (2026-06-24, app.js + styles.css; pre-edit critique = 3-lens READ-ONLY panel, verdict "portable as a focused delta, not a rewrite"): the builder dropped a new React `ScanCardFront.jsx` (developed/Halo Mint front) and asked to adopt it, KEEP the original halo, and keep dimensions "similar to existing." ~85% of the JSX already mapped 1:1 onto the live `renderCard` markup (head/house/rarity, photo+overlays, titleblock+archline, verdict, signature, mintstrip+barcode, corners, sparkles) — so the real delta is TWO things. **(1) Frame Reading pip-ledger** replaces the 4-up `.statzone`/`.ministat` bars: each row is NAME · 5 rotate-45 diamond pips · tier WORD (`.framereading`/`.fr__*`). Driven by the SAME `tierBand()` the Metrics tab uses (NOT the JSX's `tierOf` 82/65/58/50 scale, which disagreed with the live 83/66/48/25 cuts and would have contradicted the reel). No 0-100 — pips + word only. **(2) Sealed finish** (`.cardfinish`): caramel coat + lamination sweep, mix-blend overlay, gated to minted states (OFF on free). **Halo:** kept the live `.card__halo` + shiny conic edge + sparkles; DROPPED the JSX's conic `haloEdge`/`haloShadow`. **Treatment-aware:** `--fr-on` token retints the on-pips per skin — the 5 old `.ministat__fill` retints (mint, cold-foil, black-star, night-gloss, shiny→`color-mix(--halo-a)`) re-pointed to it; the develop-reveal + reduced-motion snap re-pointed `.ministat__fill`→`.fr__pip.is-on`; peak-gold/charged-purple accents GATED off free so the archive pull stays matte (pip count + word still carry the tier). **Fit:** the vertical ledger added ~60px, so `--photo-ratio` 4/3.5→4/3 (still landscape, no per-photo re-tune needed) + section-spacing trims hold the card at 777px ≈ the prior 779px (fully visible at 1600×900; same footprint). Title kept at 32px (JSX's 42px would overflow). **Cleanup:** removed the now-dead `miniStat()` + `BAND_PCT`/`bandPct` + `.ministat*` CSS + `.ministat__val--tier`. **Verified** (headless 1600×900 + DOM across menu / src1·src5 / free·shiny·mint+cold-foil; console clean): ledger pips match tierBand (Peak=5 gold, Charged=4 purple on shiny; silver+matte on free); free = ARCHIVE EDITION + FREE PULL serial + finish opacity 0; cold-foil retints pips #cfe0f4 + finish on; menu sample card renders the ledger un-clipped at zoom .92; develop flip free→shiny runs (halo spins, pips reveal, no error). ONE commit on main.

- **BR-S118 — Surface Record locked to the horizontal "tray" form (every card) + Metrics reel clipping fix + sleeker Diagnostic Receipts head + left-panel type tune** (2026-06-23, app.js + styles.css + data.js; pre-edit critique = 4-lens READ-ONLY panel, verdict proceed-with-change): resolves the BR-S117 builder choice (column vs tray → **tray wins, locked on all 5 cards**). **Surface Record (dossier plate 01):** deleted the column "vessel" render branch + the `surfaceVariant` data field (5 sources) + the column-only canvas paths (`srMakeBlobs` density/radius ternaries, `setup()` size fallbacks + gradient axis, `draw()` motion branch) — `renderSurfaceRecord` + the canvas are now tray-only. Removed the footer "Derived from sampled surface · not measured"; folded the derived honesty into the intro ("the palette **sampled off the surface**") so the per-surface % is never read as an instrument measurement (the governance lens's one push-back, honored). **Metrics reel clipping (Composition Field cut below / Frame Event bled above):** TRUE root cause (confirmed in live DOM) = the reel's flex track inherited `gap:16px` from base `.met-plates`, but the controller translates by exactly `-i*slot-h` with NO gap in the math → plate i sat `i*16px` low (Composition Field +32px, Frame Event +48px). Fixed with `gap:0` on `.met-reel .met-plates`. Also replaced the `justify-content:safe center` + `overflow-y:auto` flex-centering trap with `flex-start` + auto-margin centering (`.met-plate > :first-child{margin-top:auto}` / `:last-child{margin-bottom:auto}`): plates centre when they fit, top-anchor + fully scroll when they overflow (engine-independent, no top-bleed, bottom padding honored). **Head + type scale (the "too thick/blocky" fix — also reclaims slot height):** `.met-hd__title` 27px/600→22px/500, `.met-plate__t` 23px/600→18px/500 (head>plate hierarchy restored; the 600-weight serif was the "blocky" read), `.met-sig__class` 22→17, `.met-event__note` 19→17, mono values 11→10.5, `.met-hd__read` tracking 0.13→0.16em, head/lede/`.met-bands` margins trimmed → head 163px→148px, slot 518→533 @900. Removed the dead `.met-foot` rule. **Verified** (headless 1600×900 + DOM geometry @900 and @760; console clean across menu / src1·src2 / free·shiny / metrics): reel drift 0 on every plate; all 4 plates fit @900 (overflow 0); @760 overflowing plates top-anchor (header +15px, never negative) + scroll fully (last band reachable, ~15px breathing), short plates centre; Surface Record renders tray + canvas live (924×174, drawing) + zero "not measured" on every source; free `cfdeep` teaser path untouched. ONE commit on main. **Flagged (spawned as a follow-up task chip, NOT done here to keep this commit focused):** the remaining orphaned right-panel CSS — `.cfkey`/`.cfkey__item`, `.cfcaption`, `.module--cfield`(+`.cfbar` container), `.module--finish`+`.finish__chip/__name/__note`, `.dsource2*` — all grep-confirmed dead; a dedicated verifiable sweep removes them while KEEPING the shared `.cfbar__seg` + live `.cfdeep*`/`.surface__*`/`.metriccap`.

- **BR-S117 — fluid Surface Record (2 forms, on 2 cards to compare) + Finish/Colour-Field combined into one "Surface" module** (2026-06-23, app.js + styles.css + data.js): **Surface Record (dossier plate 01):** ported the builder's `SurfaceRecord.jsx` (React canvas) to the vanilla stack — the image's palette as live pigment suspended in developer fluid (canvas 2D + rAF, reduced-motion frozen, re-mounted after each dossier render via `mountSurfaceRecords()`). TWO forms assigned per card via `src.surfaceVariant` so they can be compared: **column** (tall glass vessel) on Driver/Shore/Tank, **tray** (wide bath) on Ice Field/Run. Paid shows the fluid + named-surface legend; free keeps the cfdeep teaser. **Combined "Surface" module (right panel):** Finish + Colour Field fused into ONE compact module (designed via a 4-agent panel — 3 sonnet + 1 opus judge; winner = "bar-as-hero"): the colour-field proportion bar is the hero, the material chip caps its left, the material name + character note form a single foot line. Free = bar + teaser (no material); paid = chip + bar + foot. One header, one R-index (was two modules). **Verified** (headless 1600×900 + canvas pixel-sample + DOM, console clean): column vessel + tray bath both render + animate; combined module compact; free states correct. ONE commit on main, pushed (live). **Builder to choose:** column vs tray Surface Record form → apply the winner to all 5. **Flagged:** the "Surface" label now shows on free (was "Colour Field") — confirm; the old `.module--finish`/`.module--cfield`/`.cfbar`(container)/`.cfcaption`/`.finish*` CSS is now orphaned (follow-up sweep).

- **BR-S116 — Reel polish + brand/Finish fixes + Metrics for 3 more cards + safe cleanup** (2026-06-23, app.js + styles.css + data.js + index.html; planned via a 15-agent workflow — 8 haiku scout / 5 sonnet build / 2 opus verify): **Reel polish** — metrics plate side padding 4px→22px (text rests, not cramped) + the left material LED softened (metPlate `metRgba(mat, 0.5→0.3)`) + landing glint whispered (met-snap 45% `.9→.34`). **Brand hitbox** — only ◆ BLUE ROOM is the menu hitbox (wrapped in `.brand__hit`); the zone sub-label is inert. **Finish reposition** — Finish sits at the TOP of the right reading panel with Colour Field directly under it (paid); Colour Field KEPT on Free (the opus gate caught the naive single-move would cut it). **Metrics for the other 3 cards** — authored `frame` for Shore (Crossbar), Run (Channel), Tank (Lantern): silhouette radii + field node + tier words + event, all safety-audited PASS (artifact/act-only, no person-read, no 0-100, tiers valid). **Safe cleanup** — removed dead JS (statDiamond, mixRow, lastH + 2 stale comments) + 7 orphaned CSS blocks (.notes family, .diagtoggle, .mixrows, .balance, .fitgrid/.fitcell, .impact__fill--dash, .notes__sub/--diagram) + junk temp files (.claude/tmp backups/scripts/dirs + a stale .bak; kept WARRANT_PEN_GROUNDTRUTH + launch.json). **Verified** (headless 1600×900 + DOM, console clean): all 5 cards' metrics render (e.g. Tank teal Lantern, Shore Crossbar); reel breathing room + soft LED; brand only-logo clickable; paid panel Finish→ColourField, free keeps ColourField. ONE commit on main, pushed (live). **Flagged follow-ups (NOT done, per the opus gate):** source-02..05.jpg image optimization (~11 MB, deliberate tooling on copies); .diamond/.dia-*/.impact CSS now orphaned by the dead-JS removal (next sweep); metriccap--foot pending a docs/map check.

- **BR-S115 — Metrics "register reel" + Source tab + photo lightbox + brand→menu + Finish/Aura split** (2026-06-23, app.js + styles.css + index.html): big interaction pass on the live left-panel tabs (reel mechanic designed via a 3-approach agent panel + judge). **Metrics = a register reel:** the "Diagnostic Receipts" header is a fixed mast; below it ONE of the 4 plates shows at a time inside a window that CLIPS while a transform-translated track moves (NOT native scroll — a nested-flex `overflow-y:auto` would not paginate here), advanced by wheel (one gesture = one step; scrolls within a tall plate first) / `◤ ◢` arrows / 4 index ticks / an `NN / 04` counter; off-centre plates dim, a source-coloured glint marks the landing, the centred plate hover-deepens; reduced-motion = instant swap; window height re-synced on resize + `fonts.ready` (NO ResizeObserver — it loops on the window here). **Tabs:** added **Source** (Source/Diagram/Metrics); removed the Diagram **Clean** toggle (Diagram is always annotated). **Source tab** = the original photo, whole + clean, click → a centred **lightbox** (X / backdrop / Esc closes; body-level so it escapes panel clip; opacity + pointer-events, hit-tested). **Brand** ◆ BLUE ROOM → main menu (`data-view-to`). **Finish** (right panel) made standalone with a per-material character line (`FINISH_NOTE`); dossier **Aura** emptied to a reserved stub. **Verified** (headless 1600×900 + DOM hit-test, console clean): reel = one bright plate + mast + controls, advances/aligns (slotH==winH, transform step matches), counter/disabled correct; Source photo + lightbox open(img topmost, centred)/close; Diagram no toggle + silver figure intact; Finish note; Aura stub; brand wired. ONE commit on main, pushed (live). *(Debug note: getComputedStyle reports composited-transition START values in this env — a red herring; real render confirmed by screenshot + hit-test.)*
