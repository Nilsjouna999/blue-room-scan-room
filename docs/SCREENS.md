...
/ `?dev=uploaded-blocked`) and the **Free Scan Simulation** (BR-S021,
`?dev=free-scan-sim`) render validated DEV fixtures and are reachable only
by an explicit query (no menu link), so they are also outside the headless
pipeline — verified live in-browser.
