...
**2026-06-25 / BR-S124 — Staged Card Reveal: built ISOLATED on a dev route; the paid "offer→modal→Halo" flow is MOCK + free-as-hero, not a paywall / ACTIVE (runtime: new `reveal/*` + index.html + app.js; refines nothing — holds the locked laws)**
Why: built the builder's staged-reveal spec (PHOTO→FREE CARD→FREE READING→warning modal→HALO CARD→HALO READING) as 7 swappable vanilla units under `window.BRReveal`, ADAPTED to the no-build stack. **TWO decisions logged with care:** (1) HOSTING — built on an isolated `?dev=staged-reveal` route (the proto-cards pattern), NOT wired onto the live menu, so the just-overhauled BR-S123 card is untouched and the surface is fully reversible; promotion to the real menu is a deliberate one-line follow-up the builder calls. (2) PAID-FLOW LANGUAGE — the spec's "purple offer → 'step into the paid version' modal → Halo" brushes the no-payment (engine) + no-magnetism laws; held LAW-FIT by: MOCK only (no real payment/engine/AI — like the existing dossier mock), and FREE-AS-HERO discipline — purple appears ONLY after the free read settles (a quiet offer, never a nag), the pulse is slow + PAUSES on hover, "Alright" is the visually-easy default, the quieter "No — let me see" is the deliberate proceed, and the modal copy states the Free Card is already COMPLETE ("you do not need more"). A read-only law audit confirmed all placeholder copy is artifact-only (no person-read, no 0–100). Card states reuse the REAL `renderCard` (free/shiny) — one master base, never forked. Copy is PLACEHOLDER (builder writes final words). Runtime is isolated (CSS scoped under `.reveal-stage`/`.rv-*` + a `window.BRReveal` namespace; zero leak into the live room/menu/card, verified).
Revisit: when the builder (a) promotes the surface to the live menu — re-audit the offer/modal under the live magnetism + free-complete laws on the REAL surface (not a dev route); and (b) wires a REAL paywall behind the modal's "No — let me see" — that is gated on the unbuilt scan ENGINE + the COMMERCIAL_SPEC_V1 one-scan-one-develop model + a per-purchase $-confirm (NEVER one-click-to-charge); the mock flow must not become a real checkout without that. Final copy lands in `reveal/readings.data.js` + the modal. The isolated units are individually redesignable/swappable without touching each other.
