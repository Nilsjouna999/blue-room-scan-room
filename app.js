...
function syncCodexBall() {
  ...
  if (!dock) {
    ...
    dock.innerHTML =
      '<a class="br-ball br-ball--orange" href="codex.html?v=482" aria-label="Search the Codex">' +
        '<span class="br-ball-core" aria-hidden="true"></span><span class="br-ball-tip" aria-hidden="true">Search</span></a>' +
      '<a class="br-ball br-ball--yellow" href="codex.html?v=482" aria-label="Open the Codex — the archive of meanings">' +
...
/* BR-S377 — THE FLIP: dev Blue Room ⇄ public Blue Room, standing in the same place.
   ...
   copies. */
/* BR-S378 — THREE BUILDS, one folder apart. `dev` is the workshop at the site root;
   `preview` is the public build WITH this flip, the copy you show people; `live` is the
...
function syncBuildFlip() {
  /* ★★ BR-S482 — NO PILL INSIDE A PEEK, WHICH IS WHAT WAS SPAWNING POPUPS.
     The peek shows a build in a frame — and that build renders its own pill, which
     opens its own peek, inside which sits another pill. The builder saw it as "it
     keeps popping pop ups", and that is exactly what it was: the control recursing
     into itself because nothing told the copy it was a copy.
     `peek=` in the URL is that signal, and the frame already carries it. One
     condition, at the top of the one function that builds the control. */
  if (/[?&]peek=/.test(String(location.search || ""))) return;
  var s = brBuildSides();
  if (!s) return;
...
/* BR-S204 — the Shelf edge arrows (menu slide 3, stepped past the Reading Rooms).
...
const ROOMS = [
  ...
  { key: "codex", state: "open", free: true, name: "The Codex",
    ...
    cost: "Free &middot; no account", cta: "Open the Codex", href: "codex.html?v=482" },
...
function renderVision() {
  /* BR-S393 — A TEMPORAL SKELETON UNDER THE PROSE. Nine paragraphs rendered as one
     uninterrupted level, so a reader felt the world clearly and could still leave
     unsure what exists TODAY: present rooms, future ground and standing principle all
     read at the same weight. Three quiet divisions fix it without turning a manifesto
     into a roadmap — U1 next door is the roadmap, and this page should not become a
     second one. The closing two paragraphs carry NO label on purpose: they are
     principle rather than a period, and a fourth heading would date them. */
  var body = VISION.length
    ? VISION.map(function (p) {
        return p.label
          ? '<p class="vis__mark"><span class="vis__mark__k">' + p.label + '</span>' +
              '<span class="vis__mark__s">' + p.sub + '</span></p>'
          : '<p class="vis__p">' + p + '</p>';
      }).join('')
  ...
  var give = '<section class="visgive" id="give">'
    ...
    + '<p class="vis__lede">Nothing is unlocked by this. What is free stays free, and '
    +   'anything with a price says so at its own door. This is here for anyone with '
    +   'something spare who wants to.</p>'
    ...
    /* BR-S404 — TWO CHANGES THE PROTOCOL ASKED FOR, AND BOTH ARE ONE WORD TO REVERT.
       `we` became `I`: an unexplained "we" makes the reader wonder how many people are
       in the room, and §5 of the protocol names this exact line. The maker steps
       forward once on this page, in the covenant, and this is the same voice keeping
       its promise about money.
       And the page no longer ENDS on being turned down. The line stays — it is the
       most humane thing on the page — but §3 is right that closing on a refusal leaves
       the reader's last experience as rejection. One plain sentence after it returns
       them to the archive, which is what all of this is for. */
    + '<p class="visgive__law">If it is over your means even slightly, I don&rsquo;t want it.</p>'
    + '<p class="visgive__use">Only if it&rsquo;s spare. Otherwise just use the archive &mdash; '
    +   'that&rsquo;s what it&rsquo;s for.</p>'
...
function renderAbout() {
  ...
  return '<section id="about" class="about u1" aria-label="What Blue Room is, and what is coming">'
    ...
    /* ★★ BR-S457 — THE ROOM LIST AND THE ROADMAP BOTH LEAVE U1. The builder:
       "current u1 those rooms deleted, only exist at room button. and road map on
        button pop up where the button is."

       WHY THIS IS A SUBTRACTION AND NOT A LOSS. U1 was carrying three jobs — say what
       Blue Room IS, list every open room, and lay out the roadmap — and the second and
       third both had a better address already built. The rooms have the ROOMS button,
       which opens a map of them; U1 was a second, longer list of the same doors, one
       scroll below the first. The roadmap has its own page at `roadmap/`, and a
       three-column board of unbuilt things sitting under the identity line answered a
       question nobody had reached yet. What is left is the one job only U1 can do.

       ★ THE ROOMS' DEPENDENCY WAS HANDLED FIRST, DELIBERATELY. `_rooms-u1.js` CLONED
       `#about .u1doors` to build the ROOMS sheet. Deleting this section without touching
       that module would not have degraded — it would have emptied the one surface the
       rooms were being moved TO. BR-S457 re-points it at `u1Open()`/`u1Door()`, the same
       two functions this line called, before this line goes.

       The roadmap's own column builders stay defined — the popup at the button is next
       and it wants exactly those. `open`/`free`/`coming` above are still read by the
       header's spec line, so the registry still governs what U1 says about itself.

       (Naming those builders in prose here is enough to trip build_public's "no live
       reference survives" check, which greps the source without stripping comments —
       so they are described rather than named. A comment that fails the build is a
       comment that will be deleted by whoever hits it at 2am.) */
    + '<footer class="u1foot">'
...
/* BR-S455: 420 -> 380. The class only has to outlive the longest thing it drives —
   the mean's arrival, 60ms delay + 300ms — and 420 left the read still lit for 180ms
   after the card's own 480ms turn had finished, so the words were moving under a
   settled card. 380 lands the whole gesture just inside it. */
const M2_BLOOM_MS = 380;
...
/* BR-S204 — the Shelf threshold (menu slide 3). A GATED conservation niche:
...
const CX_LEAVES =
  '<a class="seed" id="codexSeed" href="codex.html?v=482" role="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="codexBloom" aria-label="Open the Codex">'
  ...
  + '<iframe class="bloom__frame" data-src="codex.html?v=482" title="The Codex" tabindex="-1"></iframe>'
...
const MINI_LEAVES =
  ...
  + '<a class="mini__more" href="codex.html?v=482" data-codex-open>Open the full Codex &#8594;</a>'
  + '</div></div>';
    ...
    <div class="menu__inner">
      ...
      <div class="menu__controls">
        ...
        <div class="menu__portals">
          <a class="menu__codex" href="codex.html?v=482" data-codex-open><span class="menu__codex__mark" aria-hidden="true">◆</span> The Codex <span class="menu__codex__arr" aria-hidden="true">→</span></a>
          <a class="menu__codex" href="?dev=settings"><span class="menu__codex__mark" aria-hidden="true">◆</span> Settings <span class="menu__codex__arr" aria-hidden="true">→</span></a>
...
function wireMenuCodex(host) {
  ...
  function open() {
    if (isOpen) return;
    if (!ok) { location.href = "codex.html?v=482"; return; }           // no clip-path: the seal is a real door
    if (!loaded) {                                              // M4.3: cold click — wait for the frame, don't navigate
      ...
      coldT = setTimeout(function () {                          // after 6s the seal becomes the real door again, as it always was
        if (pendingOpen && !loaded) { pendingOpen = false; location.href = "codex.html?v=482"; }
      }, 6000);
      return;
    }
  ...
  seed.addEventListener("keydown", function (e) {
    if (e.key === "Enter") { e.stopPropagation(); }        // shield the global menu Enter→room; the anchor's activation fires the click
      ...
      if (ok) toggle(); else location.href = "codex.html?v=482";
    }
  });
  ...
  host.querySelectorAll("[data-codex-open]").forEach(function (el) {
    ...
    el.addEventListener("keydown", function (e) {
      if (e.key === "Enter") { e.stopPropagation(); }        // shield the global menu Enter→room; the anchor's activation fires the click
        ...
        e.stopPropagation(); e.preventDefault();
        if (!ok) { location.href = "codex.html?v=482"; return; }
        open();
      }
    });
  });
...
/* BR-S269 — THE PRIME. Why a panel switch stuttered on its FIRST frame, every time.
   ...
   paint two full-viewport panels (The Draw's monument, the Shelf) before it could
...
function _navStops() {
  ...
  if (about) {
    ...
    /* ★★ BR-S457 — THE CHECKPOINTS ARE GONE, AND MOST OF THEM WENT BY ATTRITION.
       The builder: "delete check points."

       They were already dead and nobody had noticed. This walked `.about__nugget` and
       `.about__close`, and BR-S416 stopped rendering both — grep finds only CONSUMERS of
       those two classes, no producer anywhere. So `_navStops()` has been returning TWO
       stops, not eight, for some time, and these lines were querying for nothing on every
       key press. Deleting them changes no behaviour; it removes the appearance of a
       feature that stopped existing.

       ★ WHAT DOES NOT GO IS ys[1] — THE SEAT, one line above. It is `_u1Seat()`, and the
       M1->U1 descent glide, the boot seat and the Orbit all read it. Killing the WALK is
       what was asked for; killing the THRESHOLD would be a different and much worse
       change, and the two are one line apart. `seat()` above is retained for the same
       reason a hook is: the next thing U1 wants to stop at will need it. */
  }
...
/* BR-S265 — THE FEATURED MINT FOLLOWS M1. The Shelf is reached by the see-deeper
...
/* BR-S266 — THE SHELF IS A DESTINATION, NOT A STATE. The reported bug was "hard refresh
   ...
   The Shelf is reached by the see-deeper hand-off, i.e. it is somewhere you are SENT, not
...
(function () {
  ...
  function rooms() {
    ...
    try {
      for (var i = 0; i < MENU_PANELS.length; i++) {
        ...
        /* EXISTENCE is derived from MENU_PANELS so a new room appears here for free;
           ...
           heading returned the Desk's eyebrow ("◆ BLUE ROOM") and the Shelf's
    ...
    out.push({ label: "The Codex", sub: "Every mark, defined", mark: AB_EMBLEMS.codex, rank: 1, wing: "work", href: "codex.html?v=482", kind: "link" });
