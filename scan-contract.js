...
(function () {
  ...
  var DEV_FIXTURES = {
    ...
    /* Free Scan Simulation fixture — upgraded to CARD_SYSTEM_V1 + CARD_LOGIC_V1
       (public tier bands, grounded receipts, scope line, sealed-stat reason,
       rarity reason, Reframe Map). FREE-tier; Halo stays sealed. Internal
       numbers exist but are NEVER printed on the card (tiers/bars only).
       Obviously a development fixture; never a real user scan. Passes the
       validator (all string values avoid the forbidden lexicon; no bare
       `value` key). */
